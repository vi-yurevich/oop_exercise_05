#ifndef OOP_EXERCISE_05_LIST_H
#define OOP_EXERCISE_05_LIST_H
...
class TList {
    private:
        struct TListNode {
            ...
            void PushBack(T& elem) {
                ...
                else next = std::make_unique<TListNode>(TListNode{elem, std::unique_ptr<TListNode>()});
            }

            void Insert(TListNode& pred, size_t index, const T& elem) {
                if (index == 0) {
                    pred.next = std::make_unique<TListNode>(TListNode{elem, std::move(pred.next)});
                }
                else if (next) {
    ...
    public:
        ...
        TList() : head(std::unique_ptr<TListNode>()) {}
        ...
        void PushBack(value_type& elem) {
            if (!head) {
                head = std::make_unique<TListNode>(TListNode{elem, std::unique_ptr<TListNode>()});
            }
            else {
                head->PushBack(elem);
            }
        ...
        void InsertToHead(const value_type& elem) {
            head = std::make_unique<TListNode>(TListNode{elem, std::move(head)});
        }
